			...
			<div class="search_style">
				<ul class="search_content clearfix">
					<form action="index" method="post">
						<!--  <li><input name="txt1" type="text" class="text_add"
							placeholder="输入开始时间" style="width: 250px"
							value=""
							 /></li>
						<li><input name="txt2" type="text" class="text_add"
							placeholder="输入结束时间" style="width: 250px" 
							value=""
							/></li> -->
						<li><label class="l_f">开始时间</label><input type="date"  name="txt1"/></li>
						<li><label class="l_f">结束时间</label><input type="date"  name="txt2"/></li>
						<li>
						<button type="submit" style="width: 100px"
								class="btn_search">
								<i class="icon-search"></i>按时间查询
							</button></li>
					</form>
				</ul>
			</div>
					...
					<tbody>
						<c:forEach items="${requestScope.list}" var="r">
							<tr>
								...
								<button class="btn btn-warning Order_form" onclick="member_add('信息',300,200,${r.id})">
					            <span>发货</span></button>
								</c:if>
								<c:if test="${r.status==4}">
								<button class="btn btn-warning Order_form" onclick="member_cancel(${r.id})">
					             <span>取消</span></button>
								</c:if>
								</td>
							</tr>
...
/*订单取消*/
function member_cancel(id){
	location.href="cancel?id="+id;
}
/*产品-新增*/
function member_add(title,w,h,id){
	layer_show(title,"add1?id="+id,w,h);
}
/*产品-删除*/
